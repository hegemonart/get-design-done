...
**Hone 让 AI 生成的 UI 始终贴住你的简报、设计系统、本地设计知识与质量闸门。为 Claude Code 打造,并可安装到 Codex、Cursor、Gemini、OpenCode、Copilot、Windsurf 等更多运行时。**
...
> **从 get-design-done (gdd) 重命名。** 现有安装继续可用：已弃用的 `/gdd:` 命令别名将保留 1-2 个版本。新的安装方式为 `npx hone`；`@hegemonart/get-design-done` 已在 npm 上弃用，改用 `@hegemonart/hone`。_(翻译可能滞后于英文 README。)_
...
Hone 帮助 AI 编码智能体交付真正属于你产品的 UI。

它把「把这个页面做好一点」这类宽泛请求,变成可追踪的设计工作流:简报、探索、规划、设计、验证。

与其让智能体仅凭品味即兴发挥,Hone 给它一套结构化流程、本地设计知识、项目专属记忆、可选的设计工具接入,以及发布前的验证。
...
Hone 为 AI 编码工作流补上缺失的设计纪律。它捕获问题、映射当前设计系统、规划受控的改动、以原子步骤执行,并对照简报、token、可访问性和设计质量评分标准验证结果。

幕后:64 个专用智能体、可查询的 intel 存储、按模型分层的路由,以及 39 个可选工具接入。你日常用到的,只是少数几个 `/hone:*` 命令。
...
从 [agentskills.io](https://agentskills.io) 技能注册表浏览并安装 Hone。
...
当一个页面技术上可用,但视觉上不一致、不清晰或设计不到位时,使用 Hone。
...
Hone 自带一套面向设计工作的丰富本地参考库。智能体无需依赖实时网络搜索,就能做出基本的设计判断。
...
Hone 创建一个 `.design/` 工作区,让每个周期都有据可依:
...
当出现差距时,Hone 会产出一份结构化的修复清单,而不是把审查交给感觉。

### 技能行为测试

Hone 自身的技能会在对抗性压力场景(时间压力、沉没成本、权威施压、范围最小化)下接受检验,以确认它们能守住纪律而不是退让。关于如何添加压力场景,见 [CONTRIBUTING.md](CONTRIBUTING.md)。
...
| 阶段 | 命令 | 产出 |
| --- | --- | --- |
| Brief | `/hone:brief` | 捕获设计问题 |
| Explore | `/hone:explore` | 映射 UI 系统、设计债、token、组件 |
| Plan | `/hone:plan` | 创建原子化设计任务 |
| Design | `/hone:design` | 带验证地执行任务 |
| Verify | `/hone:verify` | 审计最终结果 |
...
Hone 随附 96 个技能。这里是多数用户日常最需要的几个。完整参考见 [SKILL.md](SKILL.md)。
...
| --- | --- |
| `/hone:brief` | 捕获设计简报 |
| `/hone:explore` | 清点当前 UI 系统 |
| `/hone:plan` | 产出设计计划 |
| `/hone:design` | 执行计划 |
| `/hone:verify` | 验证结果 |
| `/hone:ship` | 准备干净的 PR 分支 |
| `/hone:next` | 自动路由到下一阶段 |
...
| 命令 | 用途 |
| --- | --- |
| `/hone:do <task>` | 自然语言路由器 |
| `/hone:fast <task>` | 聚焦的小修复 |
| `/hone:quick` | 轻量任务流程 |
| `/hone:audit` | 设计质量审计 |
| `/hone:darkmode` | 暗色模式审计 |
| `/hone:style <component>` | 组件样式交付 |
| `/hone:health` | 诊断流水线状态 |
| `/hone:progress` | 显示当前周期进度 |
| `/hone:resume` | 从 checkpoint 恢复 |

### 设计工具与交付

| 命令 | 用途 |
| --- | --- |
| `/hone:connections` | 配置可选集成 |
| `/hone:figma-extract` | 抽取 Figma 设计系统上下文 |
| `/hone:figma-write` | 把决策与状态写回 Figma |
| `/hone:handoff <bundle>` | 导入 Claude Design 导出包 |
| `/hone:sketch <idea>` | 生成多变体 HTML mockup |
| `/hone:spike <idea>` | 限时可行性尝试 |

完整命令参考:[SKILL.md](SKILL.md)

## 接入

Hone 无需外部工具即可工作,但可以接入 39 个可选集成。全部可选;任何接入不可用时,流水线会优雅降级到回退方案。
...
Hone 可安装到 14 个 AI 编码运行时:Claude Code、Codex、Cursor、Gemini CLI、OpenCode、Kilo、Copilot、Windsurf、Antigravity、Augment、Trae、Qwen Code、CodeBuddy 与 Cline。同一套源技能与智能体,由各运行时专属的转换器编译为每个运行时的原生布局(`skills/`、`command/`、`agents/` 或 `.clinerules`),因此流水线会随你在各编辑器之间迁移。
...
Hone 默认本地优先。它把项目产物写在 `.design/` 下,仅在配置后才使用可选集成,并对问题上报保持明确同意门控。

插件包含纵深防御钩子,用于保护路径、阻断危险命令、注入扫描、MCP 断路,以及预算强制。Hone 还暴露 13 个只读 MCP 工具,用于安全的项目内省。
...
**Claude Code 把代码交付出来。Hone 确保它也把设计交付出来。**
