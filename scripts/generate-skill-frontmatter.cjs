 ...
 * frontmatter block of every source/skills/<name>/SKILL.md from that manifest,
 ...
 * propagates source/skills -> skills/ + dist/claude-code/. A CI drift gate
 * (--check) keeps committed frontmatter == generated.
 *
 * Modes:
 *   (no flag)   regenerate source/skills/<name>/SKILL.md frontmatter from skills.json
...
function modeForward(check) {
  ...
  for (const id of dirs) {
    ...
    if (!rec) {
      ...
      fail(`${id}: present in source/skills but missing from skills.json — add a record (run --extract)`);
    }
    const abs = path.join(SRC, id, 'SKILL.md');
