---
name: hone-update
description: "Update hone to the latest release. Preserves .design/config.json and ./.claude/skills/."
...
Updates the `hone` plugin to the latest release (or a specific tag), preserving user-local state.
...
4. **Pull update** - run `claude plugin install hegemonart/hone` (or `claude plugin install hegemonart/hone@<tag>` when `--version <tag>` is passed). This re-syncs all plugin files from the release.
...
8. Print the new version and the changelog URL (`https://github.com/hegemonart/hone/releases`).
