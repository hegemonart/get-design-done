...
4. If `mcp__hone_status` returns a "STATE.md missing" error, print: "No STATE.md found. Run `{{command_prefix}}new-project` to initialize, or `{{command_prefix}}brief` to start the pipeline." and stop. Otherwise, skip to the routing table.
...
1. Check if `.design/STATE.md` exists.
   - **No STATE.md** → Print: "No STATE.md found. Run `{{command_prefix}}new-project` to initialize, or `{{command_prefix}}brief` to start the pipeline."
...
|---|---|
| `brief` | Run `{{command_prefix}}explore` to scan and interview |
| `explore` | Run `{{command_prefix}}plan` to create design plan |
| `plan` | Run `{{command_prefix}}design` to execute design tasks |
| `design` | Run `{{command_prefix}}verify` to audit and verify |
