...
# Get Design Done - /hone:start

**Role:** the canonical 0→1 proof path. A new user runs `/hone:start`, answers five short questions, and receives `.design/START-REPORT.md` with three concrete findings in the user's own code, one `best_first_proof` selected by a deterministic rubric, and a single next command to run.

**Non-goals:** do NOT write/mutate `.design/STATE.md`, enter the pipeline state machine, modify source code, auto-install MCPs or run `/hone:connections`, or capture before/after screenshots (that belongs to the full pipeline).
...
- `.design/STATE.md` already exists - route to `/hone:progress` instead.
- User asked for a full audit - route to `/hone:scan`.
- User asked to fix a specific file - route to `/hone:fast`.
...
| 5 | Print one-line handoff with suggested command (fallback: `/hone:brief` if `bestFirstProofId` is null); emit `## START COMPLETE` | `start-procedure.md#step-5-print-the-handoff` |
