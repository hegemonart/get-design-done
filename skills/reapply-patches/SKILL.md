...
Re-applies user customizations to `reference/*.md` files after `/hone:update` resyncs the plugin. Customizations are detected by diffing each `reference/` file against a pristine baseline stored in `.gdd-baseline/`.

## Steps

1. **Detect baseline** - check for `.gdd-baseline/reference/`. If it does not exist, warn the user and offer to initialize it from the current `reference/` state (no diffs will be generated on this run; the next `/hone:update` will use the new baseline).
