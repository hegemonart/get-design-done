...
4. **Check paused status via MCP**: call `mcp__hone_state__get` and inspect `status`. If it does **not** start with `paused:`, print "No pause to resume" and exit - the prior session was not paused via `/hone:pause`, so there is nothing to restore.
