...
| D-04 | **Cursor Marketplace: publisher-application gated, manual-review window.** Per 28-8-02 research (2026-05-19), Cursor Marketplace is **public-channel marketplace-UI-only** — no install-by-URL CLI. Anyone may submit a publisher application at `cursor.com/marketplace/publish`, but approval is gated by Cursor team review with no published SLA. Manifest format `.cursor-plugin/plugin.json` confirmed per `cursor.com/docs/reference/plugins`. Team Marketplaces (Team/Enterprise plan) support install-by-GitHub-URL but are private — separate channel from public marketplace, not in 28.8 scope. | Wave B (B1+B2) still ships the manifest + bundling code; B2 doctor reports application-submitted/approved status. Maintainer submits publisher application as part of post-merge step per D-09 (revised). Originally claimed "Maintainer has access" — partially refuted; submission is open but approval gate exists. |
...
| D-14 | **Codex catalog file: reuse existing `.claude-plugin/marketplace.json`.** Per 28-8-03 research (2026-05-19), Codex's `codex plugin marketplace add owner/repo` accepts `.claude-plugin/marketplace.json` (which we already ship for Claude Code marketplace) as a legacy-compatible catalog file listing plugins. We do NOT need to author a separate `.codex-plugin/marketplace.json` catalog — install-by-URL works against our existing Claude marketplace file out-of-the-box. C1 still adds `.codex-plugin/plugin.json` (plugin-shape manifest, additive to AGENTS.md per D-05) — but the catalog discovery seam is free. | Reduces C1 scope; one less file to author + maintain. Cross-marketplace install reuse is a structural advantage of Phase 28's marketplace.json design. |
| D-15 | **Cursor manifest format confirmed: `.cursor-plugin/plugin.json`.** Per 28-8-02 research (2026-05-19) sourcing `cursor.com/docs/reference/plugins`, Cursor explicitly documents the `.cursor-plugin/plugin.json` manifest format with full schema (15 fields). Earlier-session memory note suggesting "no separate manifest exists" is RETRACTED — superseded by primary docs. B1 implements per documented schema. | Source-of-truth re-verify discipline (D-07) caught a stale memory observation. Cursor docs are authoritative. |
| D-16 | **D-09 revised — Cursor field-test changes from immediate to multi-step.** Original D-09 read: "maintainer runs `cursor marketplace publish` post-merge". Revised per 28-8-02: (i) maintainer submits publisher application at `cursor.com/marketplace/publish` post-merge; (ii) awaits Cursor team review approval (no published SLA); (iii) on approval, publishes the manifest from `.cursor-plugin/plugin.json` via marketplace UI. Doctor (B2) reports application status: `not-submitted` / `submitted-pending` / `approved-published` / `rejected`. Codex field-test (per D-03) remains single-step. | Honest accounting of review-window gate. v1.28.8 ships all code + manifest; Cursor live-discovery may lag by review-window duration. Doctor mode is the maintainer's status-check surface. |
