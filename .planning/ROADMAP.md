...
**Success Criteria** (what must be TRUE):
  ...
  7. **Cost telemetry** — `.design/telemetry/costs.jsonl` appends per-agent-spawn: `{ts, agent, tier, tokens_in, tokens_out, cache_hit, est_cost_usd, cycle, phase}`. Every phase run logs. Dataset for Phase 12's reflector.
  8. **`.design/agent-metrics.json` tracker** aggregates telemetry per agent: actual duration, gap-rate, deviation-rate, context-cost. Updated incrementally. Read by Phase 12's reflector to propose frontmatter updates.
  ...
  12. **Measured outcome**: baseline pipeline run on `test-fixture/` costs ≤50% of pre-Phase-11 baseline for the same work, with no regression on DESIGN-VERIFICATION.md gap count. Evidence captured in `test-fixture/baselines/phase-11/cost-report.md`.
**Plans**: 6 plans

Plans:
- [ ] 11-01-PLAN.md — `gdd-router` skill + `.design/budget.json` schema + PreToolUse hook enforcing tier overrides, cap checks, and cached-answer short-circuits (OPT-01, OPT-02, OPT-03)
- [ ] 11-02-PLAN.md — `gdd-cache-manager` skill + `/gdd:warm-cache` command + `.design/cache-manifest.json` + cache-aware agent-prompt ordering convention documented in `agents/README.md` (OPT-04, OPT-05)
- [ ] 11-03-PLAN.md — Model-tier audit: add `default-tier` + rationale to every agent's frontmatter; extract shared cached preamble; document tier-selection guide in `reference/model-tiers.md` (OPT-06, OPT-07)
- [ ] 11-04-PLAN.md — Lazy checker spawning: Haiku gate agents for design-verifier / security-audit / UI-check triggers; streaming synthesizer pattern for parallel-mapper + parallel-researcher orchestrators (OPT-08)
- [ ] 11-05-PLAN.md — Cost telemetry (`.design/telemetry/costs.jsonl`) + `.design/agent-metrics.json` tracker + `/gdd:optimize` recommendation command + cost-report generation for baselines (OPT-09, OPT-10)
- [ ] 11-06-PLAN.md — **Phase closeout**: bump version to v1.0.5; refresh README.md (optimization layer section, budget config, warm-cache command, optimize command, model tiers); refresh plugin.json + marketplace.json (version, description, keywords — add "cost-optimization", "cache-aware", "budget"); append CHANGELOG.md v1.0.5 entry; lock regression baseline at `test-fixture/baselines/phase-11/` including `cost-report.md` (MAN-11, MAN-12)
...
**Depends on**: Phase 10 (consumes `.design/intel/` + `.design/learnings/` — without learnings, reflection has nothing to reflect on). Phase 11 (consumes `.design/telemetry/costs.jsonl` + `.design/agent-metrics.json` — reflector proposes frontmatter updates from measured data, so Phase 11's measurement layer must exist first).
...
**Success Criteria** (what must be TRUE):
  ...
  2. **Frontmatter feedback loop**: reflector reads Phase 11's `.design/agent-metrics.json` and proposes updates to agent frontmatter (`typical-duration-seconds` from measured data, `default-tier` downgrades when Haiku proved sufficient, `parallel-safe` downgrades if conflicts seen, `reads-only: false` if write patterns detected). User reviews via `/gdd:apply-reflections` — diff + confirm.
...
Plans:
- [ ] 12-01-PLAN.md — `design-reflector` agent + `.design/reflections/` artifact layout + `/gdd:reflect` command + integration with `/gdd:audit` (SLF-01, SLF-02)
- [ ] 12-02-PLAN.md — Frontmatter feedback-loop proposer (reads Phase 11's agent-metrics.json; proposes `typical-duration-seconds`, `default-tier`, `parallel-safe`, `reads-only` updates); budget-config feedback-loop proposer (SLF-03, SLF-04)
- [ ] 12-03-PLAN.md — Reference-update proposer (N-cycle pattern detection → reference/ file additions); `/gdd:apply-reflections` review + apply command; user-review discipline (SLF-05, SLF-06)
- [ ] 12-04-PLAN.md — Discussant question-quality feedback loop; global skills layer at `~/.claude/gdd/global-skills/`; promotion flow from project-local to global (SLF-07, SLF-08)
- [ ] 12-05-PLAN.md — **Phase closeout**: bump version to v1.0.6; refresh README.md (self-improvement section, reflector flow, global skills, reference-update proposer, budget feedback loop); refresh plugin.json + marketplace.json (version, description, keywords — add "self-improvement", "reflection"); append CHANGELOG.md v1.0.6 entry; lock regression baseline at `test-fixture/baselines/phase-12/` (MAN-11, MAN-12)
