...
const BASELINE_DIR_30_5 = path.join(REPO_ROOT, 'test', 'fixtures', 'baselines', 'phase-30.5');
...
test('30.5-03 baseline: phase-30.5 + prior phase manifests-version.txt match VERSION', () => {
  ...
  for (const phaseDir of PRIOR_PHASE_DIRS) {
    const f = path.join(REPO_ROOT, 'test', 'fixtures', 'baselines', phaseDir, 'manifests-version.txt');
    if (!fs.existsSync(f)) {
