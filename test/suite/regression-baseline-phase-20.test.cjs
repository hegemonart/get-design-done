...
//
// Captures the post-Phase-20 repo surface in `test/fixtures/baselines/phase-20/`
// and asserts future commits don't accidentally delete or silently rename
// anything. Complements the cross-phase `test/fixtures/baselines/current/`
...
const BASELINE_DIR = path.join(REPO_ROOT, 'test', 'fixtures', 'baselines', 'phase-20');
...
test('phase-20 baseline: all 7 snapshot files exist', () => {
  ...
  for (const f of expected) {
    assert.ok(
      ...
      `missing phase-20 baseline: test/fixtures/baselines/phase-20/${f}`,
    );
  }
});
