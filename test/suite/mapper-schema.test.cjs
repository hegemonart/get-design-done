...
for (const mapper of MAPPERS) {
  test(`mapper-schema: ${mapper.name} fixture exists and parses as valid JSON`, () => {
    const fixturePath = path.join(REPO_ROOT, 'test', 'fixtures', 'mapper-outputs', mapper.fixture);
    assert.ok(
      fs.existsSync(fixturePath),
      `test/fixtures/mapper-outputs/${mapper.fixture} must exist as schema ground truth for ${mapper.name}`
    );
