...
const STYLES = require('../../test/fixtures/figma/styles-response.json');
const NODES = require('../../test/fixtures/figma/nodes-response.json');
...
test('31-03: fixtures live at the expected test/fixtures/figma path (offline, no network)', () => {
  // Guards the key_link: the node-fetcher stub is driven by nodes-response.json.
  const stylesPath = path.join(__dirname, '../..', 'test', 'fixtures', 'figma', 'styles-response.json');
  const nodesPath = path.join(__dirname, '../..', 'test', 'fixtures', 'figma', 'nodes-response.json');
