...
const BASELINE_DIR = path.join(REPO_ROOT, 'test', 'fixtures', 'baselines', 'phase-28.5');
...
test('28.5-11 baseline: summary records 0 blockers (D-12 success criterion; warnings accepted post-Phase-32)', () => {
  ...
  assert.equal(summary.warnings, 12, `baseline records warnings=${summary.warnings}; expected 12 post-Batch-D2 (the post-Phase-52/57 skills + design/explore at 118 lines after the Batch-D2 prose-content expansion, all in the advisory >=100-line WARN band; Phase 56 override at 86 lines is clean)`);
});
