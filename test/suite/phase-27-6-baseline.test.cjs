...
const BASELINE_DIR = path.join(REPO_ROOT, 'test', 'fixtures', 'baselines', 'phase-27-6');
...
test('27.6-baseline: agent-list + hook-list baselines updated', () => {
  const agentsP20 = fs.readFileSync(
    path.join(REPO_ROOT, 'test', 'fixtures', 'baselines', 'phase-20', 'agent-list.txt'),
    'utf8',
  ).split(/\r?\n/).filter(Boolean);
  const hooksP20 = fs.readFileSync(
    path.join(REPO_ROOT, 'test', 'fixtures', 'baselines', 'phase-20', 'hook-list.txt'),
    'utf8',
  ).split(/\r?\n/).filter(Boolean);
