...
test('42-ph-01: scripts/skill-templates mirrors the skills/ skill set', () => {
  assert.deepEqual(skillDirs(SRC), skillDirs(SKILLS));
  assert.equal(walkMd(SRC).length, walkMd(SKILLS).length);
});

test('42-ph-02: every placeholder USED in scripts/skill-templates is documented in the catalogue', () => {
