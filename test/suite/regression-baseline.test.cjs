...
test('baseline: skill-list matches committed skill-templates/ directories', () => {
  // v1.58.0: skills/ is gitignored (generated by prepare/prepack); the editable source
  // (and therefore the canonical skill-list) lives at skill-templates/.
  const expected = readBaselineLines('skill-list.txt');
  const actual = gitTrackedSubdirs('skill-templates/');
  assert.deepEqual(actual, expected,
