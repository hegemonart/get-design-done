'use strict';
/**
 * Plan 30-04 — behavioral test suite for the /hone:report-issue flow.
 *
...
function freshRoot() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'hone-report-issue-test-'));
  return dir;
}
...
function defaults(overrides) {
  return Object.assign({
    ...
    submitFn: (args) => {
      ...
      return { url: 'https://github.com/hegemonart/hone/issues/123', repo: destination.DESTINATION_REPO };
    },
    stdout: sinkStdout(),
  }, overrides || {});
}
...
test('30-04 [D-03] C4: TTY answers "y" → mocked gh issue create invoked exactly once', async () => {
    ...
    options: {
      ...defaults({
        submitFn: (args) => {
          submitCalls += 1;
          captured = args;
          return { url: 'https://github.com/hegemonart/hone/issues/42', repo: destination.DESTINATION_REPO };
        },
      }),
      rootDir,
      stdin: ttyAnswer('y'),
  ...
  assert.match(result.url, /^https:\/\/github\.com\/hegemonart\/hone\/issues\//);
    ...
    options: {
      ...defaults({
        promptFn: async (opts) => {
          ...
          fs.writeFileSync(
            opts.draftPath,
            '<!-- generated by /hone:report-issue -->\n# replaced\n\nfully replaced body\n',
            'utf8'
          );
          const { title, body } = readDraft(opts.draftPath);
          return { consented: true, finalTitle: title, finalBody: body };
...
test('30-04 [D-02] H1: real gh-submit call receives --repo hegemonart/hone in argv', async () => {
  ...
  const fakeSpawn = (cmd, args) => {
    ...
    return { status: 0, stdout: 'https://github.com/hegemonart/hone/issues/100\n', stderr: '' };
  };
  ...
  assert.equal(out.url, 'https://github.com/hegemonart/hone/issues/100');
  ...
  assert.equal(capturedArgv[idx + 1], 'hegemonart/hone');
});
...
test('30-04 [D-02] H2: mutating DESTINATION_REPO at runtime throws (frozen export)', () => {
  ...
  assert.equal(destination.DESTINATION_REPO, 'hegemonart/hone');
});
...
test('30-04 [D-02] U5: draftPath bakes destination repo into the file header', () => {
  ...
  assert.match(raw, /destination: hegemonart\/hone/);
  assert.match(raw, /fingerprint: cafebabe/);
});

test('30-04 [D-11] U6: parseReportFlag honors --force-report on /hone:report-issue itself', () => {
  ...
  const result = await runReportFlow({
    ...
    options: {
      ...
      dedupCheck: ({ fingerprint, title }) => {
        ...
        return { number: 1, url: 'https://github.com/hegemonart/hone/issues/1' };
      },
    },
  });
