...
const BASELINE_DIR = path.join(REPO_ROOT, 'test', 'fixtures', 'baselines', 'phase-27-5');
...
test('27.5-baseline: skill-list baseline contains bandit-status', () => {
  const phase20 = fs.readFileSync(path.join(REPO_ROOT, 'test', 'fixtures', 'baselines', 'phase-20', 'skill-list.txt'), 'utf8')
    .split(/\r?\n/).filter(Boolean);
  const current = fs.readFileSync(path.join(REPO_ROOT, 'test', 'fixtures', 'baselines', 'current', 'skill-list.txt'), 'utf8')
    .split(/\r?\n/).filter(Boolean);
