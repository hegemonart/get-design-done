...
test('46-generator: discovery is non-vacuous (source dirs and SoT records both present)', () => {
  assert.ok(DIRS.length > 0, 'expected at least one source/skills/<id>/SKILL.md');
...
test('46-generator: every source/skills/<id> dir has exactly one skills.json record with name===id', () => {
  ...
  for (const id of DIRS) {
    ...
    assert.ok(rec, `source/skills/${id} has no record in skills.json`);
    assert.equal(rec.name, id, `skills.json record for "${id}" has name="${rec.name}"`);
  }
});

test('46-generator: every skills.json record maps to an existing source/skills/<name> dir', () => {
  const dirSet = new Set(DIRS);
  for (const rec of RECORDS) {
    assert.ok(
      dirSet.has(rec.name),
      `skills.json record "${rec.name}" has no source/skills/${rec.name}/SKILL.md`,
    );
  }
