...
const SEMVER_TEST_PATH = path.join(REPO_ROOT, 'test', 'suite', 'semver-compare.test.cjs');

const BASELINE_DIR_30 = path.join(REPO_ROOT, 'test', 'fixtures', 'baselines', 'phase-30');
...
test('30-08 baseline: prior-phase manifests-version.txt files all forward-propped to VERSION', () => {
  for (const dir of PRIOR_PHASE_DIRS) {
    const filePath = path.join(REPO_ROOT, 'test', 'fixtures', 'baselines', dir, 'manifests-version.txt');
    assert.ok(fs.existsSync(filePath),
      `prior-phase baseline missing: test/fixtures/baselines/${dir}/manifests-version.txt`);
    const got = readText(filePath).trim();
    assert.equal(got, VERSION,
      `test/fixtures/baselines/${dir}/manifests-version.txt = ${got} != VERSION ${VERSION}`);
  }
});
