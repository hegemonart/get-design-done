...
test('pipeline-smoke: test-fixture has a runnable project shape', () => {
  const pkgPath = path.join(REPO_ROOT, 'test', 'fixtures', 'package.json');
  const srcDir = path.join(REPO_ROOT, 'test', 'fixtures', 'src');
  assert.ok(fs.existsSync(pkgPath), 'test/fixtures/package.json must exist');
  assert.ok(fs.existsSync(srcDir), 'test/fixtures/src/ must exist');
  assert.ok(fs.statSync(srcDir).isDirectory(), 'test/fixtures/src must be a directory');

  // Verify at least one source file exists under src/
  const srcFiles = fs.readdirSync(srcDir);
  assert.ok(srcFiles.length > 0, 'test/fixtures/src/ must contain at least one file');
});
...
test('pipeline-smoke: current baseline manifest exists', () => {
  const currentDir = path.join(REPO_ROOT, 'test', 'fixtures', 'baselines', 'current');
  assert.ok(fs.existsSync(currentDir), 'test/fixtures/baselines/current/ must exist');
  ...
  assert.ok(
    hasManifest,
    'test/fixtures/baselines/current/ must contain a manifest (BASELINE.md, README.md, or a *-list.txt file)'
  );
});
