...
//
// MCP server `hone-state` — exposes the 11 typed STATE.md tools implemented
 ...
 * from the repo root by convention, so `sdk/mcp/hone-state`
 * resolves reliably. Both branches are canonicalized against the
 * on-disk tools directory.
 */
function here(): string {
  const expectedRel = join('sdk', 'mcp', 'hone-state');
 ...
 * our file. A direct `node sdk/mcp/hone-state/server.ts`
...
function isMain(): boolean {
  ...
  return /sdk\/mcp\/hone-state\/server\.(ts|js|cjs|mjs)$/.test(
    entry.replace(/\\/g, '/'),
  );
}
...
if (isMain()) {
  runStdio().catch((err) => {
    ...
    console.error(`[hone-state] fatal: ${msg}`);
    process.exit(1);
  });
}
