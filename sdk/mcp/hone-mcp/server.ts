...
// sdk/mcp/hone-mcp/server.ts
//
// MCP server `hone-mcp` — read-mostly project-state surface (Phase 27.7).
...
// (Moved from scripts/mcp-servers/hone-mcp/ to sdk/mcp/hone-mcp/ in Plan
//  31-5-05 for MCP-server symmetry with sdk/mcp/hone-state/ — D-08.)
//
// Lifecycle (mirrors Phase 20 `hone-state` server):
...
//      Phase 52 raised it to 13 (hone_context_query, D5).
...
export const SERVER_NAME = 'hone-mcp';
 ...
 * from the repo root by convention, so `sdk/mcp/hone-mcp`
 * resolves reliably. Both branches are canonicalized against the
 * on-disk tools directory.
 */
function here(): string {
  const expectedRel = join('sdk', 'mcp', 'hone-mcp');
 ...
 *  `schemaPath` pointing into `sdk/mcp/hone-mcp/schemas/`. */
 ...
 * (hone_context_query) — one per tool name in the canonical 13-tool list.
 */
export const TOOL_DESCRIPTIONS: Record<string, string> = {
  hone_status:
    'hone_status: current cycle phase, branch, last-3 decisions, last-3 completed plans, blocker count.',
  hone_context_query:
    'hone_context_query: read-only query over the DesignContext graph (.design/context-graph.json) — op one of nodes/edges/path/consumers-of/unreachable/cycles/coverage; structured no-graph result when absent.',
  hone_phase_current:
    'hone_phase_current: STATE.md <position> block (phase, stage, task_progress, status).',
  hone_phases_list:
    'hone_phases_list: parsed ROADMAP.md overview (phase number, name, target version, shipped/planned).',
  hone_plans_list:
    'hone_plans_list: plans tracked in STATE.md must_haves (optionally filtered by input.phase).',
  hone_decisions_list:
    'hone_decisions_list: decisions from STATE.md (optionally filtered by input.status).',
  hone_intel_get:
    'hone_intel_get: read a slice from .design/intel/ (input.slice_id required); directory_not_found if absent.',
  hone_telemetry_query:
    'hone_telemetry_query: typed reader over .design/telemetry/*.jsonl with type/since/limit filters.',
  hone_cycle_recap:
    'hone_cycle_recap: diff current STATE against latest .design/snapshots/ snapshot; directory_not_found if absent.',
  hone_reflections_latest:
    'hone_reflections_latest: newest .design/reflections/ file (excerpt <= 4 KB); directory_not_found if absent.',
  hone_learnings_digest:
    'hone_learnings_digest: aggregate last N reflections into a compact digest (<= 5 KB).',
  hone_events_tail:
...
export const TOOL_READONLY: Record<string, boolean> = {
  hone_status: true,
  hone_context_query: true,
  hone_phase_current: true,
  hone_phases_list: true,
  hone_plans_list: true,
  hone_decisions_list: true,
  hone_intel_get: true,
  hone_telemetry_query: true,
  hone_cycle_recap: true,
  hone_reflections_latest: true,
  hone_learnings_digest: true,
  hone_events_tail: true,
  hone_health: true,
};
...
export function buildServer(): Server {
  ...
  for (const t of tools) {
    try {
      ...
      console.error(
        `[hone-mcp] schema compile failed for ${t.name}; tool degraded to permissive validation: ${msg}`,
      );
 ...
 * our file. A direct `node sdk/mcp/hone-mcp/server.ts`
...
function isMain(): boolean {
  ...
  return /sdk\/mcp\/hone-mcp\/server\.(ts|js|cjs|mjs)$/.test(
    entry.replace(/\\/g, '/'),
  );
}
...
if (isMain()) {
  runStdio().catch((err) => {
    const msg = err instanceof Error ? err.message : String(err);
    // eslint-disable-next-line no-console
    console.error(`[hone-mcp] fatal: ${msg}`);
    process.exit(1);
  });
}
