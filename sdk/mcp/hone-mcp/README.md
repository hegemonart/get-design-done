...
Phase 27.7. Mirror of the `sdk/mcp/hone-state/` Phase 20 pattern. v1 is read-only — mutation belongs to slash-skills and the lockfile-safe `hone-state-mcp` writers (Phase 20 surface).
...
| `hone_phase_current` | `{}` | Current phase + stage + task_progress + status from STATE.md `<position>` |
| `hone_phases_list` | `{}` | Phases from ROADMAP.md overview parse |
| `hone_plans_list` | `{phase?}` | Plans for the current (or specified) phase from STATE.md `<plans>` |
| `hone_decisions_list` | `{status?}` | D-XX decisions from STATE.md `<decisions>`, optionally filtered |
| `hone_intel_get` | `{slice_id, shape?}` | Slice query against `.design/intel/` with optional key projection |
| `hone_telemetry_query` | `{type?, since?, limit?}` | Typed reader over `.design/telemetry/*.jsonl` |
| `hone_cycle_recap` | `{since_snapshot?}` | Diff vs the last Phase 27.6 snapshot (STATE sections + decisions + plans) |
| `hone_reflections_latest` | `{}` | Latest cycle reflection excerpt from `.design/reflections/` (≤4 KB) |
| `hone_learnings_digest` | `{cycles?}` | Compact lessons digest of recent reflections (≤5 KB) |
| `hone_events_tail` | `{type?, limit?}` | Last N events from event-chain with optional type filter |
| `hone_health` | `{}` | Read-only mirror of `skills/health/SKILL.md` checks payload |
