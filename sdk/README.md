...
| --------------- | --------------------------------------------------- | ------------------------------------------------------------------------------------- | --------- |
| cli             | `@hegemonart/hone/sdk/cli`               | `dispatch`, `main`, `USAGE`, `DispatcherDeps`                                          | beta      |
| state           | `@hegemonart/hone/sdk/state`             | `read`, `mutate`, `transition`, `ParsedState`, `Stage`                                | beta      |
| event-stream    | `@hegemonart/hone/sdk/event-stream`      | `appendEvent`, `getWriter`, `getBus`, `subscribe`, `subscribeAll`, `reset`, `readEvents`, `aggregate`, `EventBus`, `EventWriter`, `KNOWN_EVENT_TYPES` | beta      |
| errors          | `@hegemonart/hone/sdk/errors`            | `GDDError`, `ValidationError`, `StateConflictError`, `OperationFailedError`, `TransitionGateFailed`, `LockAcquisitionError`, `ParseError` | beta      |
| primitives/error-classifier   | `@hegemonart/hone/sdk/primitives/error-classifier`   | `classify`, `FailoverReason`                              | stable    |
| primitives/iteration-budget   | `@hegemonart/hone/sdk/primitives/iteration-budget`   | `consume`, `refund`, `remaining`, `reset`, `IterationBudgetExhaustedError` | stable    |
| primitives/jittered-backoff   | `@hegemonart/hone/sdk/primitives/jittered-backoff`   | `delayMs`, `sleep`, `DEFAULTS`                            | stable    |
| primitives/lockfile           | `@hegemonart/hone/sdk/primitives/lockfile`           | `acquire`, `renameWithRetry`                              | stable    |
| mcp/hone-state   | `@hegemonart/hone/sdk/mcp/hone-state`     | `buildServer` (server.ts) — 11 STATE.md tools over stdio                              | beta      |

The barrel: `import { read, appendEvent, ValidationError } from '@hegemonart/hone/sdk'`.
