...
const BASELINE_DIR_288 = path.join(REPO_ROOT, 'test-fixture', 'baselines', 'phase-28.8');
const BASELINE_DIR_287 = path.join(REPO_ROOT, 'test-fixture', 'baselines', 'phase-28.7');
const BASELINE_DIR_286 = path.join(REPO_ROOT, 'test-fixture', 'baselines', 'phase-28.6');
const BASELINE_DIR_285 = path.join(REPO_ROOT, 'test-fixture', 'baselines', 'phase-28.5');
const BASELINE_DIR_28 = path.join(REPO_ROOT, 'test-fixture', 'baselines', 'phase-28');
...
describe('Phase 28.8-Z1: CHANGELOG + OFF_CADENCE registration', () => {
  ...
  test('28.8-Z1: CHANGELOG.md has ## [VERSION] - 2026-05-19 block at top (within first 8 lines)', () => {
    const head8 = read('CHANGELOG.md').split(/\r?\n/).slice(0, 8).join('\n');
    // CHANGELOG format: ## [<version>] - YYYY-MM-DD (hyphen separator per plan spec).
    const re = new RegExp(`^## \\[${escapeRegExp(VERSION)}\\]\\s+-\\s+2026-05-19`, 'm');
    assert.match(head8, re, `CHANGELOG head 8 lines missing ## [${VERSION}] - 2026-05-19 block`);
  });
});
