...
version: 1.0.0
phase: 28.5
tags: [scan, procedure, extracted, pipeline-stage, design-system, audit]
...
```
Tip: run {{command_prefix}}connections to walk through setup for Figma, Storybook, Chromatic, and 9 other integrations.
```

Suppress the nudge if any probe returned `available` or `unavailable`, or if the user has already run `{{command_prefix}}connections` (marker: `connections_onboarding` block present or previously drained).
...
|---|---|---|---|
| body | [Npx] | [N] | [check/cross] |
| sm | [Npx] | [N] | [check/cross] |
...
**Core primitives present:** [Button check, Input check, Modal cross, Toast cross, ...]
...
> Quick wins (P1 + XS/S effort) are marked with lightning bolt.
